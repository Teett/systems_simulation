...
```{r, echo=FALSE}
tiempos %>% 
  count(servicio, sort = T)
```

A general try to the aggregated services: Note that this is incorrect, as triage dynamics are hidden inside the data.
...
Changing the scale of the data doesn't affect it's distribution as long as it remains positive and every number is scaled. This helps the Maximum likelihood Stimation to converge.
...
Lognormal distribution may work, need further verification. Theorically there is no distribution that can fit these data, as there are distinguishable categories that behave differently inside the data. Separation by service and triage is needed.

Therefore, the following code creates subsets that only include the particular services of interest.
...
```{r}
fit_adultos_exp <- fitdist(tiempos_adultos$dur_servicio/250, distr = "exp", method = "mle")
fit_adultos_exp
...
```{r}
fit_adultos_gamma <- fitdist(tiempos_adultos$dur_servicio/300, distr = "gamma", method = "mle")
fit_adultos_gamma
...
```{r}
fit_llegadas_weibull <- fitdist(as.numeric(tiempo_llegadas)/250, distr = "weibull")
...
```{r}
fit_llegadas_gamma <- fitdist(as.numeric(tiempo_llegadas)/200, distr = "gamma", method = "mle")
fit_llegadas_gamma
...
```{r}
fit_llegadas_lognormal <- fitdist(as.numeric(tiempo_llegadas)/200, distr = "lnorm", method = "mle")
...
```{r}
fit_llegadas_exp <- fitdist(as.numeric(tiempo_llegadas)/250, distr = "exp")
fit_llegadas_exp
